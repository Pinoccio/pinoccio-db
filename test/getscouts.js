    ...
    d.writeScout(troop.id,{name:"stu"},function(err,scout){
      ...
      d.writeScout(troop.id,{},function(err,scout){
        d.get(troop.id,function(err,troop){
          var scouts = troop.scouts;

          t.ok(!err,'should not have error getting scouts');
          t.equals(Object.keys(scouts).length,2,'should have 2 scouts.');
          t.equals(scouts[1].id,1,'scout id should be 1');
          t.equals(scouts[2].id,2,'scout id should be 2');
          t.end();
        })
      });
    })
